...
基于 Visual Studio 新扩展性模型（VisualStudio.Extensibility）的扩展，目标是简化 Dynamics 365/Dataverse 与 On‑Premise 环境中的插件程序集发布与部署。当前版本已在项目节点添加“插件发布”入口，后续将逐步完善环境选择与自动发布流程。
...
- 进行中 / 计划中
  ...
  - 多步骤配置流程：环境选择 → 认证方式 → 连接配置 → 确认发布 ✅
  - 认证方式：OAuth、连接字符串、AD、IFD（界面已完成，待集成具体认证逻辑）
